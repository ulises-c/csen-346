#!/usr/bin/env bash
# Run KELE eval: AMD Linux teacher + two Mac Mini mlx_lm consultants (parallel workers).
#
# Worker 1 → MAC1 (even-indexed dialogues)
# Worker 2 → MAC2 (odd-indexed dialogues)
#
# Prerequisites (run on each Mac Mini before starting this script):
#   source configs/consultants/m4-mlx.env
#   ./scripts/serve_consultant_mlx.sh
#
# Usage:
#   ./scripts/eval_amd_2mac_mlx.sh                   # full run
#   ./scripts/eval_amd_2mac_mlx.sh --limit 10        # smoke test (10 total, 5 per worker)
#   ./scripts/eval_amd_2mac_mlx.sh --limit 10 --new  # fresh run, archives previous results
#
...
# ── Logging ───────────────────────────────────────────────────────────────────
log() { printf '[%s] %s\n' "$(date -Iseconds)" "$*" | tee -a "$ORCH_LOG"; }

log "=== KELE Dual-Mac MLX Evaluation: $EXPERIMENT ==="
...
if ! curl -sf "${MAC1_URL}/models" > /dev/null; then
    log "ERROR: Mac Mini 1 unreachable. Run serve_consultant_mlx.sh on it first."
    exit 1
fi
log "Mac Mini 1 OK."
...
if ! curl -sf "${MAC2_URL}/models" > /dev/null; then
    log "ERROR: Mac Mini 2 unreachable. Run serve_consultant_mlx.sh on it first."
    exit 1
fi
log "Mac Mini 2 OK."
...
log "Waiting for teacher to be ready..."
...
# ── Launch workers ────────────────────────────────────────────────────────────
log "Launching worker 1 (→ $MAC1_URL, even-indexed dialogues) ..."
...
log "Launching worker 2 (→ $MAC2_URL, odd-indexed dialogues) ..."
...
aggregate_progress() {
    while true; do
        local c1=0 t1=0 c2=0 t2=0 line rest
