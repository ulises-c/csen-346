#!/usr/bin/env bash
# Orchestrator for the Gemma 4 31B local evaluation run.
...
#
# Usage:
#   ./scripts/eval_gemma4_31b.sh smoke              # n=5
#   ./scripts/eval_gemma4_31b.sh mini               # n=25
#   ./scripts/eval_gemma4_31b.sh full               # n=681
#   ./scripts/eval_gemma4_31b.sh smoke --keep-server  # leave server running
#   ./scripts/eval_gemma4_31b.sh smoke --no-compare   # skip baseline comparison
#   ./scripts/eval_gemma4_31b.sh smoke --unified      # fusion architecture (single-call)
#
...
#
# Server policy: if a server is already serving "Gemma 4 31B" on port 8080,
...
# ── Mode config ───────────────────────────────────────────────────────────────
# Output dir suffix:
#   (default)    results/gemma4-31b-local-{smoke,mini}  /  results/gemma4-31b-local
...
case "$MODE" in
  smoke) N=5;   OUT_DIR="results/gemma4-31b-local-smoke${SUFFIX}"; SUBCMD="test"     ;;
  mini)  N=25;  OUT_DIR="results/gemma4-31b-local-mini${SUFFIX}";  SUBCMD="test"     ;;
  full)  N=0;   OUT_DIR="results/gemma4-31b-local${SUFFIX}";       SUBCMD="evaluate" ;;
esac
...
# ── Pre-flight ────────────────────────────────────────────────────────────────
echo "=== Gemma 4 31B Eval Orchestrator ==="
...
preflight_check "weight file"         "$GEMMA4_31B_WEIGHTS_DIR/$WEIGHT_FILE"
...
if ready_check; then
  ...
  if probe_server >/dev/null; then
    ...
    bash "$ROOT/scripts/serve_gemma4_31b_q5.sh" > "$SERVER_LOG" 2>&1 &
    SERVER_PID=$!
    WE_BOOTED=true
  fi
