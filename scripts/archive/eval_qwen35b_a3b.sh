...
#
# Server policy: if a server is already serving "Qwen 35B A3B" on port 8080,
...
preflight_check "weight file"         "$QWEN35B_WEIGHTS_DIR/$WEIGHT_FILE"
...
if ready_check; then
  ...
  if probe_server >/dev/null; then
    ...
    bash "$ROOT/scripts/serve_qwen35b_a3b.sh" > "$SERVER_LOG" 2>&1 &
    SERVER_PID=$!
    WE_BOOTED=true
  fi
