#!/usr/bin/env bash
# Orchestrator for the Qwen 35B-A3B (MoE) local evaluation run.
# MoE architecture: 256 experts, 9 active per token, ~3B active of 35B total.
# Expected ~3-4x faster than dense Qwen 27B; ~3x more KV-efficient.
#
...
#
# Usage:
#   ./scripts/eval_qwen35b_a3b.sh smoke              # n=5,   ~2 min  (projected)
#   ./scripts/eval_qwen35b_a3b.sh mini               # n=25,  ~5 min  (projected)
#   ./scripts/eval_qwen35b_a3b.sh full               # n=681, ~20-30 h (projected)
#   ./scripts/eval_qwen35b_a3b.sh smoke --keep-server  # leave server running
#   ./scripts/eval_qwen35b_a3b.sh smoke --no-compare   # skip baseline comparison
#
# Server policy: if a server is already serving "Qwopus 35B A3B" on port 8080,
...
#
# NOTE: shares port 8080 with serve_qwen27b_q5.sh — only one model fits at a
# time on the 5090's 32 GB VRAM. Stop any 27B server before invoking this one.
...
EXPERIMENT="qwopus35b-a3b-local"
SUFFIX=""
if $UNIFIED; then
  SUFFIX="${SUFFIX}-unified"
fi
if $NOTHINK; then
  EXPERIMENT="qwopus35b-a3b-local-nothink"
  SUFFIX="${SUFFIX}-nothink"
fi
case "$MODE" in
  smoke) N=5;   OUT_DIR="results/qwopus35b-a3b-local-smoke${SUFFIX}"; SUBCMD="test"     ;;
  mini)  N=25;  OUT_DIR="results/qwopus35b-a3b-local-mini${SUFFIX}";  SUBCMD="test"     ;;
  full)  N=0;   OUT_DIR="results/qwopus35b-a3b-local${SUFFIX}";       SUBCMD="evaluate" ;;
esac

# ── Constants / env-overridable paths ─────────────────────────────────────────
LLAMA_SERVER="${LLAMA_SERVER:-$HOME/Documents/models/llama.cpp/build/bin/llama-server}"
QWOPUS_WEIGHTS_DIR="${QWOPUS_WEIGHTS_DIR:-$HOME/Documents/models/weights}"
WEIGHT_FILE="${QWOPUS_WEIGHT_FILE:-Qwopus3.6-35B-A3B-v1-Q4_K_M.gguf}"
EXPECTED_ALIAS="Qwopus 35B A3B"
PORT="${PORT:-8080}"
...
# ── Pre-flight ────────────────────────────────────────────────────────────────
echo "=== Qwopus 35B-A3B Eval Orchestrator ==="
...
preflight_check "weight file"         "$QWOPUS_WEIGHTS_DIR/$WEIGHT_FILE"
...
if ready_check; then
  ...
  if probe_server >/dev/null; then
    ...
    bash "$ROOT/scripts/serve_qwopus35b_a3b_think.sh" > "$SERVER_LOG" 2>&1 &
    SERVER_PID=$!
    WE_BOOTED=true
  fi
...
if command -v systemd-inhibit &>/dev/null; then
  INHIBIT="systemd-inhibit --what=sleep:idle --who=eval_qwen35b_a3b.sh --why=KELE-eval"
fi
