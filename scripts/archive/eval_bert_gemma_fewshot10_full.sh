#!/usr/bin/env bash
# End-to-end orchestrator for the BERT + Gemma 4 31B + 10-shot exemplars
# integration at n=681 (full test split).
#
# This is the headline candidate run kicked off 2026-05-17 after the standalone
# Gemma 4 31B full run underperformed (31.39% / 21% schema fallback). BERT
# replaces the consultant, removing the dependency on Gemma's JSON adherence.
# 10-shot stage-balanced exemplars in the teacher prompt recover surface form.
#
# n=50 wall clock was 53 min @ ~60 dlg/hr. Full run projects ~11-14 h.
#
# Usage:
...
echo "=== BERT + Gemma 4 31B + 10-shot full eval ==="
echo "Started: $(date)"
echo "Output:  $OUT_DIR"
echo "BERT:    $BERT_CKPT"
...
echo "=== Eval ==="
echo "Time: $(date)"
echo "Experiment: $EXPERIMENT (BERT consultant + Gemma teacher + 10-shot)"
...
# Override via env, e.g.:  KELE_PARALLEL_WORKERS=4 bash scripts/eval_bert_gemma_fewshot10_full.sh
